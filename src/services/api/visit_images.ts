...
export class VisitImageService {
    ...
    async fetchChanges(timestamp: string, visit_image_ids: number[], offset: number, location_id?: number): Promise<DeltaChangesResponse> {
        const url = `/api/appv2/fetchHelperData/visit-images`;
        try {
            const response = await this.api.post<DeltaChangesResponse>(url, { location_id, timestamp, visit_image_ids, offset });
            return response.data;
        } catch (error: any) {
