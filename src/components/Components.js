    ...
    <View
      ...
      {styles && sizes
        ? names.map((iconName, index, []) => {
          return (
            <View
              key={iconName + Math.random().toString()}
              style={{ ...styles[index], position: 'absolute' }}>
...
export const ImageWithEditableRemark = ({
  item,
  displayString,
  onChangeRemark,
  onDelete,
    ...
    <View
      ...
      <View
        ...
        <Image
          ...
          style={{ width: 100, height: 100 }} // Set your desired image dimensions and margin
        />
        <Text style={{ ...commonStyles.text3,color: 'black', textAlign: 'center' }}>
          {displayString}
        </Text>
        <TouchableOpacity
          onPress={() =>
