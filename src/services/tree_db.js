...
export class LocalDatabase {
  ...
  logExceptionLocalDB = async logs => {
    try {
      const phoneinfo =
        ...
        (await AsyncStorage.getItem(Constants.phoneNumber));
  ...
  getAllTrees = async () => {
    try {
      ...
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedGetTreedata', Strings.english),
      );
  ...
  getTreeBySaplingID = async saplingId => {
    ...
    const results = await this.db.executeSql(queryString, [saplingId]);
    results.forEach(result => {
      for (let index = 0; index < result.rows.length; index++) {
        trees.push(result.rows.item(index));
      }
  ...
  getTreeImages = async saplingid => {
    try {
      ...
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedGetTreeImages', Strings.english),
      );
      const stackTrace = error.stack;
      const errorLog = {
  ...
  getAllTreeCount = async () => {
    try {
      ...
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedGetTreeCount', Strings.english),
      );
      const stackTrace = error.stack;
      const errorLog = {
  ...
  setFalse = async () => {
    try {
      let res = await this.db.executeSql(
        `update ${treeTableName} set uploaded=0`,
      );
      return res[0].rows.length;
    } catch (error) {
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedSetFalse', Strings.english),
      );
      const stackTrace = error.stack;
      const errorLog = {
  ...
  saveTreeImages = async treeimage => {
    const insertQuery =
      ...
      `('${treeimage.saplingid}', '${treeimage.image}', '${
        treeimage.imageid
      }', '${treeimage.remark.replace("'", "''")}', '${treeimage.timestamp}')`;
  ...
  createTreetTypesTbl = async () => {
    ...
    const query1 = `DROP TABLE IF EXISTS ${treetypeName};`;
    await this.db.executeSql(query1);
  ...
  updateTreeTypeTbl = async treeTypesInLocalDBFormat => {
    if (
        !Array.isArray(treeTypesInLocalDBFormat) ||
        treeTypesInLocalDBFormat.length === 0
      ) {
        console.log('empty treetypes array in tree_db.js');
      }

      //delete existing records
    //console.log("------deleting earlier treeTYpes------",treeTypesInLocalDBFormat[0]._id)
    // await this.db.executeSql(`DELETE FROM ${treetypeName}`);
     const values = treeTypesInLocalDBFormat
      .map(treetype => `('${treetype.name}', '${treetype.tree_id}', '${treetype._id}')`)
      .join(',');

    const insertQuery =
      `INSERT OR REPLACE INTO ${treetypeName}(name, value,_id) VALUES ${values}`;

      // const insertQuery =
    //     `INSERT OR REPLACE INTO ${treetypeName}(name, value) values` +
    //     `('${treetype.name}', '${treetype.tree_id}')`;
    return this.db.executeSql(insertQuery);
  };
  ...
  getTreeTypesUsedByLocalTrees = async () => {
    ...
    try {
      ...
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedGetTreeNames', Strings.english),
      );
      const stackTrace = error.stack;
      const errorLog = {
  ...
  getPlotNamesUsedByLocalTrees = async () => {
    ...
    try {
      ...
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedGetPlotNames', Strings.english),
      );
      const stackTrace = error.stack;
      const errorLog = {
  ...
  getSaplingIds = async () => {
    ...
    try {
      let res = await this.db.executeSql(
        `SELECT saplingid as name FROM ${treeTableName}`,
      );
      res.forEach(result => {
        for (let index = 0; index < result.rows.length; index++) {
      ...
      console.error(error);
      Alert.alert(
        Strings.alertMessages.getString('FailedGetSaplingIds', Strings.english),
      );
      const stackTrace = error.stack;
      const errorLog = {
  ...
  createPlotTbl = async () => {
    ...
    const query1 = `DROP TABLE IF EXISTS ${plotName};`;
    await this.db.executeSql(query1);
  ...
  updatePlotTbl = async plotsInLocalDBFormat => {
    if (
      !Array.isArray(plotsInLocalDBFormat) ||
      plotsInLocalDBFormat.length === 0
    ) {
      console.log('empty plot array in tree_db.js');
    }

    //delete existing records
    // console.log("------deleting earlier plots------")
    // await this.db.executeSql(`DELETE FROM ${plotName}`);

     const values = plotsInLocalDBFormat
      .map(plot => `('${plot.name}', '${plot.plot_id}','${plot._id}')`)
      .join(',');

    // Constructing the insert query
    const insertQuery = `INSERT OR REPLACE INTO ${plotName} (name, value, _id) VALUES ${values}`;
    //
    // const insertQuery =
  ...
  updateSaplingTbl = async saplingDocs => {
    ...
    const values = saplingDocs
      .map(saplingDoc => `('${saplingDoc.saplingid}')`)
      .join(',');
  ...
  storePlotSaplings = async (plot_id, saplings) => {
    let insertQuery = `INSERT OR REPLACE INTO sapling_plot(plotid,saplingid ,lat, lng) VALUES`;
  ...
  getSaplingsforPlot = async plotId => {
    ...
    try {
      let res = await this.db.executeSql(
        `SELECT * FROM sapling_plot WHERE plotid = '${plotId}'`,
      );
      res.forEach(result => {
        for (let index = 0; index < result.rows.length; index++) {
          saplings.push(result.rows.item(index));
