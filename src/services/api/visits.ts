...
export class VisitService {
    ...
    async fetchChanges(timestamp: string, visit_ids: number[], offset: number, location_id?: number): Promise<VisitHelperDataResponse> {
        const url = `/api/appv2/fetchHelperData/visits`;
        try {
            const response = await this.api.post<VisitHelperDataResponse>(url, { location_id, timestamp, visit_ids, offset });
            return response.data;
        } catch (error: any) {
